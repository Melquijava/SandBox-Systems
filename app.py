...
# --- Rotas de Autenticação (com alteração no redirect) ---
...
def login():
    if request.method == 'POST':
        ...
        # ALTERAÇÃO: Redireciona para o novo dashboard, não mais para o editor
...
@app.route('/dashboard')
def dashboard():
    """NOVA ROTA: A tela principal com a lista de projetos."""
    ...
    # Passamos os projetos para o novo template 'dashboard.html'
    return render_template('dashboard.html', projects=user_projects, username=session['username'])

@app.route('/view/<project_id>')
def view(project_id):
    """NOVA ROTA: Visualização pública de um projeto."""
    users_data = load_data()
    found_project = None
    # Procura o projeto em todas as contas de usuário
...
# --- Rotas do Editor e API (sem grandes alterações) ---
